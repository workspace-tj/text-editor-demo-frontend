...
function TableCellResizer({
  ...
  const mouseUpHandler = useCallback(
      ...
      const handler = (event: MouseEvent) => {
        ...
        if (mouseStartPosRef.current) {
          ...
          if (isHeightChanging(direction)) {
            const height = activeCell.elem.clientHeight;
            const heightChange =
              Math.abs(event.clientY - y) / zoom / magnification;
            ...
            const widthChange =
              Math.abs(event.clientX - x) / zoom / magnification;
    ...
    [activeCell, resetState, updateColumnWidth, updateRowHeight, magnification],
  );
...
export default function TableCellResizerPlugin({
  ...
  return useMemo(
    () =>
      isEditable
        ? createPortal(
            <TableCellResizer editor={editor} magnification={magnification} />,
            document.body,
          )
        : null,
    [editor, isEditable, magnification],
  );
}
